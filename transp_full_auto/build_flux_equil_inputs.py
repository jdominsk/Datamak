...
def create_gk_studies(conn: sqlite3.Connection, origin_name: str) -> None:
    ...
    conn.execute(
        ...
        WHERE de.active = 1
          ...
          AND gs.id IS NULL
        """,
        (gk_code_id_val, gk_code_id_val, origin_name),
    )
...
def create_gk_inputs(
    ...
    study_ids = fetch_active_studies(conn, origin_name)
    if not study_ids:
...
def populate_equil_and_timeseries(
    ...
    origin_id, origin_path = get_origin(conn, origin_name)
    ...
    for idx, name in enumerate(files, start=1):
        ...
        ts_key = (origin_id, shot_number, shot_variant)
        ...
        if ts_key not in existing_ts:
            ...
            row = conn.execute(
                ...
                WHERE data_origin_id = ?
                  ...
                  AND shot_variant IS ?
                """,
                (origin_id, shot_number, shot_variant),
            ).fetchone()
            if row and row[0]:
    ...
    if files:
        print()
    if create_studies:
        create_gk_studies(conn, origin_name)
    return inserted_equil, inserted_ts


def main() -> None:
    args = parse_args()
    if args.create_gk_inputs and not args.origin_name:
        raise SystemExit("--origin-name is required with --create-gk-inputs")
    if args.populate_equil and not args.origin_name:
        raise SystemExit("--origin-name is required with --populate-equil")
    if args.create_studies and not args.origin_name:
        raise SystemExit("--origin-name is required with --create-studies")
    ...
    if args.flux_db:
        ...
        try:
            ...
            if args.populate_equil:
                inserted_equil, inserted_ts = populate_equil_and_timeseries(
                    dst,
                    args.origin_name,
                    args.remote_path,
                    args.create_studies,
                ...
                print(
                    ...
                    f"transp_timeseries={inserted_ts} for {args.origin_name}"
                )
